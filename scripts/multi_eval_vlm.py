...
parser.add_argument("--model", required=True, choices=["llava", "blip", "qwen", "gpt4o"])
parser.add_argument("--dataset_root", default="vista_dataset")
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"vista_results_{MODEL_TYPE}.csv")
ERROR_FILE = os.path.join(OUTPUT_DIR, f"vista_results_{MODEL_TYPE}_errors.csv")
...
GPT4O_MAX_RETRIES = 3
GPT4O_RETRY_DELAY = 5  # seconds; doubles each retry
    ...
    for attempt in range(GPT4O_MAX_RETRIES):
        try:
            ...
            if attempt < GPT4O_MAX_RETRIES - 1:
                ...
                print(
                    f"[GPT-4o] {type(e).__name__} — retrying in {wait}s "
...
def run_model(frames, prompt):
    ...
    if MODEL_TYPE == "llava":
        ...
        raise ValueError(f"Unsupported model type in run_model: {MODEL_TYPE}")
...
def evaluate():
    ...
    for clip in tqdm(clips):
        try:
            with open(clip["json"], "r", encoding="utf-8") as f:
                data = json.load(f)
            ...
            guidance_gt = " ".join(a.get("instruction", "") for a in actions if isinstance(a, dict)).strip()
            ...
            errors.append({"model": MODEL_TYPE, "clip_id": clip["clip_id"], "error": str(e)})
    ...
    _, _, scene_f1 = bert_score_batch(
        df["scene_pred"].fillna("").tolist(),
        df["scene_gt"].fillna("").tolist(),
        lang="en",
        device=DEVICE,
    )

    _, _, guidance_f1 = bert_score_batch(
        df["guidance_pred"].fillna("").tolist(),
        df["guidance_gt"].fillna("").tolist(),
        lang="en",
        device=DEVICE,
    )
