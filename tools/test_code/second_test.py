...
for frame_idx in range(num_images):
    ...
    camera_inv_crf_table = np.linspace(0, 1, 256)
    linear_img = camera_inv_crf_table[rot_img]

    srgb_img = linear_to_srgb(linear_img)
    srgb_uint8 = (srgb_img * 255).astype(np.uint8)

    timestamp_ns = image_data[1].capture_timestamp_ns

    img_filename = f"{meta['id']}_{frame_idx:02d}.jpg" #Save Image
    cv2.imwrite(os.path.join(images_folder, img_filename), srgb_uint8)
